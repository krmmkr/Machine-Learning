...
###########################################################################################################
dataset1 = 'C:/Users/kohul/OneDrive/Documents/ML/student-mat.csv'
...
##############################Data Set 1#################################################
#Data Processing
Y=data1.iloc[:,32:33].values
Y = (np.where(Y>np.median(Y),1,0))
data1 = data1.drop(['G1','G2','G3'],axis=1)
data1 = Check(data1)
X = createDummy(data1)
print(X.columns)
...
for i in range(1,20):
    ...
    clf = AdaBoostClassifier(n_estimators=200, base_estimator=dt,learning_rate=0.01)
    clf.fit(X_train_1,Y_train_1)
    y_pred_boost = clf.predict(X_test_1)
...
for i in range(1,100):
    dt = DecisionTreeClassifier(criterion = "gini", max_depth = 2,random_state = 100) 
    clf = AdaBoostClassifier(n_estimators=200, base_estimator=dt,learning_rate=i/100)
    clf.fit(X_train_1,Y_train_1)
    y_pred_boost = clf.predict(X_test_1)
    accuracy_test.append(accuracy_score(Y_test_1,y_pred_boost)*100)
    lr.append(i/1000)
